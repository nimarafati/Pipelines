## New functions with sample_correlation
```{r new_functions, eval = F, echo = T}
Functions
```{r functions, echo = T, eval = T}
## Running GO enrichmment
run_GO_enrichment <- function(path, contrast, functional_info){
  setwd(paste0(path,  '/intermediate/DE_chromosome_level_Trimmed/', contrast))
  Analysed_genes <- read.table(paste0(path, '/intermediate/DE_chromosome_level_Trimmed/', contrast, '/logFC-CPM-FDR.txt'), header = T, sep ='\t')
  ...
  write.table(Analysed_genes[,c('gene_id')], paste0(path, '/intermediate/DE_chromosome_level_Trimmed/', contrast, '/logFC-CPM-FDR_GO_Terms.txt'), col.names = F, sep ='\t', quote = F, row.names = F)
  
  #select DE egnes that have GO terms
  DE_edgeR_results <- read.table(paste0(path, '/intermediate/DE_chromosome_level_Trimmed/', contrast, '/DE-edgeR-results.txt'), header = T, sep ='\t')
  ...
  write.table(DE_edgeR_results[,c('gene_id')], paste0(path, '/intermediate/DE_chromosome_level_Trimmed/', contrast, '/DE-edgeR-results_GO_Terms.txt'), col.names = F, sep ='\t', quote = F, row.names = F)
  #Run goatools
  writeLines(text = 'ln -s ~/Dropbox/NBIS/Projects/SMS_5221_20_SounDrivenBiotechnology/data/meta_data/annotation/GO_Association.txt
ln -s /Users/nimra236/anaconda3/bin/go-basic.obo
ln -s /Users/nimra236/anaconda3/bin/goslim_generic.obo
~/Software/goatools/scripts/find_enrichment.py DE-edgeR-results_GO_Terms.txt logFC-CPM-FDR_GO_Terms.txt GO_Association.txt --outfile=goea_results.xlsx,goea_results.tsv --pvalcalc fisher --pval 0.05', con = 'goatools.sh') 
  cat(paste0('cd ', path,  '/intermediate/DE_chromosome_level_Trimmed/', contrast,'\n', 'sh goatools.sh'))
  }else{
...
sample_correlation_heatmap <- function(data.TMM.matrix){
    ...
    data_r=matrix(0,nrow(FinalDE),length(rep_names),byrow = T)
...
run_edge_R <- function(data.TMM, A, B, annotation, group, nsample){
  ...
  source("~/Dropbox/Pipelines/RNA-seq/rnaseq_plot_funcs.R") #These are already written functions to color significant genes by raw p_value or FDR
  ...
  if(sum(DE_summary[c(1,3)])>=2){
    ...
    data.TMM.matrix = FinalDE[,c(9:ncol(FinalDE))] # remove the gene column since its now the rowname value
    ...
    heatmap.2(data.TMM.matrix, dendrogram='row', Rowv=as.dendrogram(hc_genes), Colv=F, 
              col=myheatcol, RowSideColors=gene_colors, scale="none", density.info="none", trace="none", key=TRUE, keysize=1.2, cexCol=1, lmat=rbind(c(5,0,4,0),c(3,1,2,0)), lhei=c(1.5,5),lwid=c(1.5,0.2,2.5,1.5), margins=c(5,2), breaks=seq(quantBrks[1], quantBrks[2], length=76),main="log2 (normalized expression + 1)")
    dev.off()

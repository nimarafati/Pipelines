...
while(<inSample>)
{
	...
	if($data_type eq "PE")
	{
		$QC1.="fastqc --extract --contaminants ~/glob_old/Contamination/adapters-tab-txt.list -o $lineArr[0]  -t 2 $lineArr[3]/$lineArr[1] $lineArr[3]/$lineArr[2] &\n";
	}
	else
	{
		$QC1.="fastqc --extract --contaminants ~/glob_old/Contamination/adapters-tab-txt.list -o $lineArr[0]  -t 2 $lineArr[3]/$lineArr[1] &\n";
	}
}
...
while(<inSample>)
{
	...
	$trim.="cp /home/nimar/glob_old/Contamination/adapters.fa adapters.fa \n";
	...
	if($data_type eq "PE")
	{
		...
		$rev=$1;
		$trim.="java -jar ~/glob_old/Software/Trimmomatic-0.30/trimmomatic-0.30.jar PE -threads 2 -phred33 -trimlog $lineArr[0].log  $lineArr[3]/$lineArr[1] $lineArr[3]/$lineArr[2] $for-paired.fq $for-unpaired.fq $rev-paired.fq $rev-unpaired.fq ILLUMINACLIP:adapters.fa:2:40:15:8:true SLIDINGWINDOW:4:15 LEADING:15 TRAILING:15 MINLEN:36 &\n";
	}
	else
	{
		$trim.="java -jar ~/glob_old/Software/Trimmomatic-0.30/trimmomatic-0.30.jar SE -threads 2 -phred33 -trimlog $lineArr[0].log  $lineArr[3]/$lineArr[1] $for-trimmed.fq ILLUMINACLIP:adapters.fa:2:40:15:8:true SLIDINGWINDOW:4:15 LEADING:15 TRAILING:15 MINLEN:36 &\n";
	}
}
...
while(<inSample>)
{
	...
	if($data_type eq "PE")
	{
		$lineArr[2]=~ m/(.*)\.fastq.gz/;
		$rev=$1;
		$QC2.="fastqc --extract --contaminants ~/glob_old/Contamination/adapters-tab-txt.list -o $lineArr[0]  -t 2 $cWD/Trimmed-reads/$lineArr[0]/$for-paired.fq $cWD/Trimmed-reads/$lineArr[0]/$rev-paired.fq &\n";
	}
	else
	{
		$QC2.="fastqc --extract --contaminants ~/glob_old/Contamination/adapters-tab-txt.list -o $lineArr[0]  -t 2 $cWD/Trimmed-reads/$lineArr[0]/$for-trimmed.fq &\n";
	}
}
...
#	print "$path_to_ref/$reference_name.fai";<STDIN>;
	...
	Sbatch_run($gsnap,"$lineArr[0]-aln",$proj,12*60);
	$gsnap="";
}
close inSample;
##Prepare bam files
...
#	Sbatch_run($prep,"$lineArr[0]-prep",$proj,24*60);
	Sbatch_run_core($prep,"$lineArr[0]-prep",$proj,1*60);
	$prep="";
}
close inSample;
##Run cufflinks
...
cd $cWD/cufflinks\n";
	if($gtf ne "")
	{
		...
		{
			$cufflinks_CMD.="cufflinks -p 16 -o $lineArr[0] -u --library-type fr-firststrand -g $gtf $cWD/GSNAP/$lineArr[0]/$lineArr[0]-gsnap.bam";
		}
#/proj/b2012206/private/nobackup/annotation/Broad_annotation_2013_10_15/Updated_annotations_dec13/rabbit_annotation/Broad-131213-All-annotations.gtf /proj/b2012206/private/nobackup/UserDirectories/nima/Domestication/GSNAP/$a/$a-gsnap-snpT.Reordered.sort.bam";
	...
	else
	{
		if($vcf ne "")
		{
			$cufflinks_CMD.="cufflinks -p 16 -o $lineArr[0] -u --library-type fr-firststrand $cWD/GSNAP/$lineArr[0]/$lineArr[0]-gsnap-snpT.Reordered.sort.bam";
		}
		else
		{
			$cufflinks_CMD.="cufflinks -p 16 -o $lineArr[0] -u --library-type fr-firststrand $cWD/GSNAP/$lineArr[0]/$lineArr[0]-gsnap.bam";
		}
	}
	Sbatch_run($cufflinks_CMD,"$lineArr[0]-cufflinks",$proj,6*60);
}
close inSample;
##cuffmerge

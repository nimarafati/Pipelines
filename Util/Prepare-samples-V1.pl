...
my $usage="Prepare-samples-V1.pl -path 
...
if($path eq "" || $reference eq "" || $gtf eq "" )
{
...
while(<inSample>)
{
	...
	chomp($_);
	@lineArr=split("\t",$_);
	$QC1.="fastqc --contaminants ~/glob/Contamination/adapters-tab-txt.list -o $lineArr[0] --noextract -t 2 $lineArr[3]/$lineArr[1] $lineArr[3]/$lineArr[2] &\n";
}
...
while(<inSample>)
{
	...
	$lineArr[2]=~ m/(.*)\.fastq.gz/;
	$rev=$1;
	$QC2.="fastqc --contaminants ~/glob/Contamination/adapters-tab-txt.list -o $lineArr[0] --noextract -t 2 $cWD/Trimmed-reads/$lineArr[0]/$for-paired.fq $cWD/Trimmed-reads/$lineArr[0]/$rev-paired.fq &\n";
}
